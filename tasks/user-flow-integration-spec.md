...
가입 이후 글 작성, 반응, 댓글, 신고, 대화, 공유, 프로필 확인이 Supabase에 저장되고 관리자 화면까지 이어지는 닫힌 사용자 흐름을 만든다. 공개 앱은 카카오 로그인만 유지하되 개발 빌드에서는 별도 개발 계정으로 실제 Supabase 세션을 만들 수 있어야 한다.

## Assumptions

- 공개 인증은 카카오 전용이며 개발용 이메일 로그인은 `__DEV__`에서만 노출한다.
...
- 개발 계정과 카카오 계정 모두 Supabase 세션을 사용하고 웹 재실행 후 세션이 유지된다.
