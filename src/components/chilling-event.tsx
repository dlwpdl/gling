...
export function ChillingKindTabs({ value, onChange }: { value: ChillingKind; onChange: (kind: ChillingKind) => void }) {
  ...
  return <View accessibilityRole="tablist" accessibilityLabel="만남 유형" style={[styles.tabs, { backgroundColor: theme.backgroundElement }]}>
    {(['once', 'group'] as const).map(kind => <Pressable key={kind} accessibilityRole="tab" accessibilityState={{ selected: value === kind }} onPress={() => onChange(kind)} style={[styles.tab, value === kind && { backgroundColor: theme.background }]}><ThemedText type="smallBold">{kind === 'once' ? '칠링 (일회성)' : '모임 (정기모임)'}</ThemedText></Pressable>)}
  </View>;
}
...
export const ChillingEvent = memo(function ChillingEvent({ post, city, onOpen, onAuthor }: {
  ...
  return <View style={styles.card}>
    <Pressable onPress={onOpen} accessibilityRole="button" accessibilityLabel={`${post.title}, ${chillingSchedule(post.room ?? {})}`} style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1 })}>
    ...
    <View style={styles.footer}>
      <Pressable onPress={onAuthor} accessibilityRole="button" accessibilityLabel={`${post.author.nickname} 프로필`} style={styles.author}><ThemedText type="small" themeColor="textSecondary">{post.author.nickname}님이 열었어요 ›</ThemedText></Pressable>
      {post.author.id !== me.id && <Pressable accessibilityRole="button" accessibilityLabel="신고 및 차단" style={styles.action} onPress={() => isAuthed ? setReport(true) : promptLogin('신고하려면 로그인해 주세요.')}><ThemedText style={{ color: theme.textSecondary }}>···</ThemedText></Pressable>}
    </View>
