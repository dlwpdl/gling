-- Add Google to the existing social-only auth hooks; retain review/admin restrictions.
create or replace function public.gling_before_user_created(event jsonb)
returns jsonb language sql stable set search_path = '' as $$
  select case when
    event #>> '{user,app_metadata,provider}' in ('kakao', 'apple', 'google')
    or event #> '{user,app_metadata,review_access}' = 'true'::jsonb
    or event #>> '{user,app_metadata,role}' = 'admin'
  then '{}'::jsonb
  else '{"error":{"http_code":403,"message":"Please use Kakao, Apple, or Google sign-in. Public email signup is not available."}}'::jsonb end;
$$;
...
begin
  ...
  if metadata ->> 'role' = 'admin'
    ...
    or (metadata ->> 'provider' in ('kakao', 'apple', 'google') and
