...
select ('73000000-0000-0000-0000-'||lpad(n::text,12,'0'))::uuid,'72000000-0000-0000-0000-000000000002','vancouver',5,'구독 한도 모임 '||n,'함께 산책할 사람을 구해요.',current_date-n,'{"capacity":8}'::jsonb from generate_series(1,2)n;
...
select is((public.get_membership()->>'meetupsUsed')::integer,2,'pending request leaves occupied slots unchanged');
reset role;
...
select set_config('request.jwt.claims','{"sub":"72000000-0000-0000-0000-000000000002","role":"authenticated"}',true);
set local role authenticated;
select is((public.get_membership()->>'meetupsUsed')::integer,2,'one owned and one joined consume exactly two');
...
select is((public.get_membership()->>'conversationsUsed')::integer,2,'prior-day active rooms still consume slots');
...
set local role authenticated;
select lives_ok(format('select public.respond_direct_conversation(%L,''accepted'')',:'direct_id'),'plus capacity permits a third concurrent room');
reset role;
