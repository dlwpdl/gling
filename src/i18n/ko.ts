...
export const t = {
  ...
  write: {
    ...
    kindListingHint: '렌트, 룸메, 중고, 차량',
    pricePlaceholder: '가격 (선택, CAD)',
    listingNote: '구해요·팔아요 글은 오늘 작성 한도를 쓰지 않아요. 14일 뒤 자동 마감되고, 하루에 한 번 끌어올릴 수 있어요.',
    listingRemaining: (used: number, max: number) => `살아있는 글 ${used} / ${max}`,
    listingBodyPlaceholder: '무엇을 구하거나 파나요? 상태, 거래 동네, 원하는 연락 방법을 적어주세요',
    bodyPlaceholder: {
  ...
  auth: {
    ...
    sessionExpired: '로그인이 풀렸어요. 다시 로그인하면 이어서 할 수 있어요',
  ...
  detail: {
    ...
    delete: '삭제',
    editing: '댓글 수정 중',
    editErrorTitle: '댓글을 수정하지 못했어요',
    deleteTitle: '이 댓글을 삭제할까요?',
    deleteBody: '삭제한 댓글은 되돌릴 수 없어요. 달린 답글은 그대로 남아요.',
    deleteErrorTitle: '댓글을 삭제하지 못했어요',
    bump: '끌어올리기',
    bumped: '끌어올렸어요',
    markPartial: '일부 판매됨',
    markClosed: '거래 완료',
    reopen: '다시 열기',
    listingBadge: '구해요·팔아요',
    listingStatus: { open: '거래 중', partial: '일부 판매', closed: '거래 완료' } as Record<string, string>,
    bumpedLabel: '끌어올림',
    price: (value: number) => `$${value.toLocaleString('en-CA', { maximumFractionDigits: 0 })}`,
    expiresIn: (days: number) => (days <= 0 ? '오늘 마감' : `${days}일 뒤 마감`),
  },
  ...
  report: {
    ...
    submitting: '접수 중…',
    doneTitle: '신고가 접수됐어요',
    doneBody: '운영팀에 신고를 전달했어요. 신고한 콘텐츠는 내 화면에서 숨겨져요. 위반이 확인되면 전체 공개를 중단합니다.',
    doneBlocked: '운영팀에 신고를 전달하고 차단했어요. 이 사용자의 콘텐츠는 내 화면에 표시되지 않아요.',
    ...
    blockBody: '이 사용자의 글·댓글·메시지를 내 화면에서 숨기고 상호작용을 막아요. 운영팀에 전달되며 원본 기록은 보존 정책에 따라 보관됩니다.',
    reasons: {
      spam: '스팸·도배',
      harassment: '괴롭힘·위협',
      hate: '혐오 표현',
