...
import { ActivityIndicator, DeviceEventEmitter, FlatList, Modal, Pressable, ScrollView, StyleSheet, View } from 'react-native';
...
function MeetupDiscovery() {
  ...
  return <SafeAreaView edges={['top', 'left', 'right']} style={[styles.root, { backgroundColor: theme.background }]}>
    <FlatList data={visible} keyExtractor={p => p.id} refreshing={loading && posts.length > 0} onRefresh={() => void fetchPage()}
      contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + TabBarHeight + 24 }]}
      ListHeaderComponent={<>
        <View style={styles.head}><View style={{ flex: 1 }}><Pressable onPress={() => setCityPicker(true)} accessibilityRole="button" accessibilityLabel="활동 지역 선택" style={styles.city}><ThemedText type="small" themeColor="textSecondary">{city.name}에서⌄</ThemedText></Pressable><ThemedText accessibilityRole="header" style={styles.title}>모임<ThemedText style={styles.title} themeColor="accent">.</ThemedText></ThemedText></View>
          <Pressable accessibilityRole="button" accessibilityLabel="내 모임 프로필" onPress={() => action('/meetup-profile')} style={[styles.circle, { backgroundColor: theme.backgroundElement }]}><ThemedText type="smallBold">{isAuthed ? me.nickname.slice(0, 1) : '나'}</ThemedText></Pressable>
          <Pressable accessibilityRole="button" accessibilityLabel="만남 열기" onPress={() => action('/meetup-create')} style={[styles.circle, { borderColor: theme.line, borderWidth: 1 }]}><ThemedText style={{ fontSize: 26 }}>＋</ThemedText></Pressable></View>
        <View style={[styles.segments, { backgroundColor: theme.backgroundElement }]}>{(['once', 'group'] as const).map(value => <Pressable key={value} accessibilityRole="tab" accessibilityState={{ selected: kind === value }} onPress={() => { setKind(value); setCategory('all'); }} style={[styles.segment, kind === value && { backgroundColor: theme.background }]}><ThemedText type="smallBold">{value === 'once' ? '칠링 (일회성)' : '모임 (정기모임)'}</ThemedText></Pressable>)}</View>
        <ThemedText themeColor="textSecondary" style={styles.lede}>{kind === 'once' ? '이번 한 번, 가볍게 만나봐요.' : '취향이 맞는 사람들과, 오래 봐요.'}</ThemedText>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chips}>{[['all','전체'],['casual','가볍게'],['hobby','취미'],['travel','여행']].map(([value,label]) => <Pressable key={value} accessibilityRole="button" accessibilityState={{ selected: category === value }} onPress={() => setCategory(value)} style={[styles.chip, { borderColor: theme.line, backgroundColor: category === value ? theme.text : theme.background }]}><ThemedText type="small" style={{ color: category === value ? theme.background : theme.text }}>{label}</ThemedText></Pressable>)}</ScrollView>
