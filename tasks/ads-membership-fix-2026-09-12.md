...
- Apple 월/연 4개 상품의 한국어·영어 설명 8개를 현재 모임·대화 자리 및 24시간 잠금 규칙으로 수정했다. 가격표 복구 후 검증에서 가격 누락은 없어졌지만 상품 상태는 모두 `MISSING_METADATA`다. 별도 API로 심사 스크린샷이 4개 모두 없음을 확인했다.
...
2. 실제 구매 가능한 멤버십 화면으로 Apple 구독 심사 스크린샷 4개를 준비하고 제출용 앱에 연결해야 한다. 검증되지 않은 구매 화면을 합성해 제출하지 않는다.
