...
rows=list(csv.DictReader((base/'calendar-after.csv').open(encoding='utf-8-sig')))
...
assert changed=={'GL-P005','GL-P006'}
...
assert len({x['threadId'] for x in mail['unique_threads']})==5 and len(mail['new_ids'])==0
body=json.loads(json.loads((base/'report-dom.json').read_text())['result']['result'])
