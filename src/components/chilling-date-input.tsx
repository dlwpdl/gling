...
export function ChillingDateInput({ label, value, onChange, disabled }: ChillingDateInputProps) {
  ...
  return <View style={{ gap: 8, marginVertical: 8 }}>
    ...
    {Platform.OS === 'ios' ? <DateTimePicker value={date} mode="datetime" display="compact" disabled={disabled} accentColor={theme.accent} locale="ko-KR" style={{ minHeight: 48, width: '100%' }} onValueChange={(_, next) => onChange(next.toISOString())} /> : <>
      <View style={{ flexDirection: 'row', gap: 12 }}>{(['date', 'time'] as const).map(part => <Pressable key={part} disabled={disabled} accessibilityRole="button" accessibilityLabel={`${label} ${part === 'date' ? '날짜' : '시간'} 선택`} onPress={() => setMode(part)} style={{ minHeight: 48, justifyContent: 'center', padding: 12, borderWidth: 1, borderRadius: 10, borderColor: theme.line }}><ThemedText>{part === 'date' ? date.toLocaleDateString('ko-KR') : date.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })}</ThemedText></Pressable>)}</View>
