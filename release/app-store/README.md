...
2026-09-10 TestFlight 확인 당시 빌드 6은 `VALID` / `READY_FOR_BETA_TESTING`이었지만 그룹과 테스터가 모두 0이었다. `Gling Internal` 내부 그룹(`03fefab3-98b9-4f9c-a6a0-450fdd703efb`)을 만들고 빌드 6을 연결했으며 원격 관계를 재조회해 확인했다. 테스터 초대 메일은 수신자 확인·승인 대기 중이다. 빌드 업로드와 테스터 배포는 별도 단계이며, 초대 수락·실기기 설치는 아직 확인하지 않았다.
